...
pile_frac_top = np.zeros((n_resol))
...
for j in range(n_resol):
    ...
    pile_frac_top[j] = combpile.P_pile_n(n=ns[j], top=True, occupancies=occupancies, degeneracies=degeneracies, aft=afts[i])

np.save('/home/yuanlq/combpile/results/phd_pile_up_frac_top_z%s.py'%(i), pile_frac_top)