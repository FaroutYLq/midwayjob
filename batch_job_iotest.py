...
class Submit(object):
    ...
    def _submit_single(self, loop_index, loop_item):
        jobname = 'iotest{:03}'.format(loop_index)
        run_id = loop_item
        # Modify here for the script to run
        jobstring = "python /home/yuanlq/software/midwayjob/iotest.py %s"%(run_id)
        print(jobstring)

        # Modify here for the log name
        utilix.batchq.submit_job(
            jobstring, log='/home/yuanlq/.tmp_job_submission/iotest%s.log'%(run_id), partition='dali', qos='dali',
