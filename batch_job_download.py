...
class Submit(object):
    ...
    def _submit_single(self, loop_index, loop_item):
        jobname = 'copy_{:03}'.format(loop_index)
        run_id = loop_item
        # Modify here for the script to run
        jobstring = "python /home/yuanlq/software/midwayjob/copy_to_frontend.py %s"%(run_id)
        print(jobstring)

        # Modify here for the log name
        utilix.batchq.submit_job(
            jobstring, log='/home/yuanlq/.tmp_job_submission/copy_peaks_63%s.log'%(run_id), partition='dali', qos='dali',
            account='pi-lgrandi', jobname=jobname,
            delete_file=True, dry_run=False, mem_per_cpu=10000,
            container='xenonnt-2022.06.3.simg',
            cpus_per_task=1)
...
"""
loop_over = ['event_info', 'event_positions', 'event_ambience',
 'events_mv', 'events_nv', 'event_w_bayes_class',
 'event_shadow', 'event_pattern_fit', 'event_basics',
 'veto_regions', 'veto_proximity', 'veto_intervals',
 'cuts_basic', 'cut_fiducial_volume', 'cut_daq_veto']
"""
#loop_over = ['events_sync_nv']
loop_over = ['peak_basics', 'peak_positions', 'peak_proximity']
